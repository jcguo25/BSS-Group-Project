...
batt.P_DischrgPackMax = batt.cell.capacity_Ah * batt.cell.CRateMaxDischrg * ...
        batt.cell.ocv_volt_axis(1) * batt.Ns * batt.Np / 1000;
% Calculate max battery discharge power [kW]
batt.P_ChrgPackMax = batt.cell.capacity_Ah * batt.cell.CRateMaxChrg * ...
        batt.cell.ocv_volt_axis(1) * batt.Ns * batt.Np / 1000;
% Calculate pack internal resistance
batt.R_int_pack = batt.cell.R_int * batt.Ns / batt.Np / 1000;   %[Ohm]
...
for i = 1:N
    ...
    [P_batt(i), P_fc(i)] = detmPowerDistribution(fcState(i), P_req(i), fc, batt);
...
energyGeneratedFC = cumsum((P_fc) .* dt) / 3600; % [kWh]
avgPwr = energyConsum(end) * 3600 / cycle500km.time(end); % [kW]
eqvlSocCycles = calcEqvlSocCycle(soc);

P_OhmicDissipation = I_pack .^ 2 .* batt.R_int_pack;         %[w]
% Calculate total ohmic dissipation energy over the mission [kWh]
totalOhmicDissipation = cumsum(P_OhmicDissipation ./ 1000 .* dt) / 3600; % [kWh]

%% Plot & Results Output
fprintf('Total energy consumption over 500 km:          %.2f kWh\n', energyConsum(end));
fprintf('Average power:                                 %.2f kW\n', avgPwr)
fprintf('Total energy generated by FC over 500 km:      %.2f kWh\n', energyGeneratedFC(end));
